export async function canvasToBlob(canvas, format = 'png', quality = 0.8) {
  return new Promise((resolve, reject) => canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('画像Blobの生成に失敗しました。')), format === 'jpeg' ? 'image/jpeg' : 'image/png', quality));
}
...
export function copyCanvas(src, width = src.width, height = src.height, alpha = false, bg = '#fff') {
...
export function trimWhitespace(src, { threshold = 18, padding = 16, background = '#fff', mode = 'white' } = {}) {
  ...
  const isMargin = (i) => {
    ...
    if (a === 0) return true;
    if (mode === 'black') return r <= threshold && g <= threshold && b <= threshold;
  ...
  if (nw < Math.max(64, w * 0.12) || nh < Math.max(64, h * 0.12)) return src;
  const canvas = document.createElement('canvas');
  canvas.width = nw;
  canvas.height = nh;
